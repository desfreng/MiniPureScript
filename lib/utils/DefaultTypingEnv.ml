...
let not_fimpl, mod_fimpl, log_fimpl, pure_fimpl, bool_show_fimpl, int_show_fimpl
    =
  let f x = "%" ^ x in
  ( f not_fun_name
  , f mod_fun_name
  , f log_fun_name
  , f pure_fun_name
  , f ("bool_" ^ show_fun_name)
  , f ("int_" ^ show_fun_name) )
  ...
  List.fold_left
    (fun acc (fid, ar, f_id) ->
      Function.Map.add fid
        ...
        ; tfun_arity= ar
        ; tfun_vars= []
        ; tfun_texpr= Either.Right f_id }
        acc )
    Function.Map.empty
    [ (not_fid, 1, not_fimpl)
    ; (mod_fid, 1, mod_fimpl)
    ; (log_fid, 1, log_fimpl)
    ; (pure_fid, 1, pure_fimpl) ]
    ...
    Schema.Map.singleton bool_show_sid
      ...
      ; tschema_funs=
          Function.Map.singleton show_fid
            ...
            ; tfun_arity= 1
            ; tfun_vars= []
            ; tfun_texpr= Either.right bool_show_fimpl } }
  in
  Schema.Map.add int_show_sid
    ...
    ; tschema_funs=
        Function.Map.singleton show_fid
          ...
          ; tfun_arity= 1
          ; tfun_vars= []
          ; tfun_texpr= Either.right int_show_fimpl } }
    default_schema_impl
